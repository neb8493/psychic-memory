...
FONT=$(find /usr/share/fonts /usr/local/share/fonts /home -iname "Montserrat-Bold*.ttf" -print -quit 2>/dev/null || true)
[ -z "$FONT" ] && FONT=$(find /usr/share/fonts /usr/local/share/fonts -iname "*Bold*.ttf" -print -quit 2>/dev/null || true)
[ -z "$FONT" ] && FONT=$(find / -iname "*.ttf" -print -quit 2>/dev/null || true)
FONTR=$(find /usr/share/fonts /usr/local/share/fonts /home -iname "Montserrat-Regular*.ttf" -print -quit 2>/dev/null || true); [ -z "$FONTR" ] && FONTR=$FONT
echo "font=$FONT" 
...
push(){ # still with slow push-in: $1 name $2 file $3 dur
  N=$(( $3 * FPS ))
  ffmpeg -nostdin -loglevel error -y -loop 1 -framerate $FPS -i "src/$2" -t $3 -vf "scale=2016:1134:force_original_aspect_ratio=increase,crop=2016:1134,zoompan=z='1+0.05*on/$N':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=${W}x${HT}:fps=$FPS,format=yuv420p" $ENC "seg/$1.mp4"
}
pan(){ # 21:9 still panned: $1 name $2 file $3 dur $4 dir (r|l)
  if [ "$4" = r ]; then X="(iw-ow)*t/$3"; else X="(iw-ow)*(1-t/$3)"; fi
  ffmpeg -nostdin -loglevel error -y -loop 1 -framerate $FPS -i "src/$2" -t $3 -vf "scale=-2:$HT,crop=$W:$HT:x='$X':y=0,format=yuv420p" $ENC "seg/$1.mp4"
}
clip(){ # $1 name $2 file $3 dur
  ffmpeg -nostdin -loglevel error -y -i "src/$2" -vf "scale=$W:$HT:force_original_aspect_ratio=increase,crop=$W:$HT,fps=$FPS,tpad=stop_mode=clone:stop_duration=3,format=yuv420p" -t $3 $ENC "seg/$1.mp4"
}
card(){ # $1 name $2 text $3 dur
  printf '%s' "$2" > seg/$1.txt
  ffmpeg -nostdin -loglevel error -y -f lavfi -i "color=c=0x1F4E9A:s=${W}x${HT}:d=$3:r=$FPS" -vf "drawtext=fontfile='$FONT':textfile=seg/$1.txt:fontcolor=0xFFD23F:fontsize=96:x=(w-tw)/2:y=(h-th)/2,fade=t=in:st=0:d=0.3,fade=t=out:st=$(awk "BEGIN{print $3-0.3}"):d=0.3,format=yuv420p" $ENC "seg/$1.mp4"
}
...
ffmpeg -nostdin -loglevel error -y -f concat -safe 0 -i seg/list.txt -c copy out/video.mp4
ffprobe -v error -show_entries format=duration -of csv=p=0 out/video.mp4
# ---- audio: music loop 8:00 + voiceover at 0:14 ----
ffmpeg -nostdin -loglevel error -y -i src/music.mp3 -af "afade=t=in:st=0:d=0.5,afade=t=out:st=119:d=2" -ar 44100 seg/m.wav
ffmpeg -nostdin -loglevel error -y -i seg/m.wav -i seg/m.wav -i seg/m.wav -i seg/m.wav -i seg/m.wav -filter_complex "[0:a][1:a]acrossfade=d=2:c1=tri:c2=tri[a];[a][2:a]acrossfade=d=2:c1=tri:c2=tri[b];[b][3:a]acrossfade=d=2:c1=tri:c2=tri[c];[c][4:a]acrossfade=d=2:c1=tri:c2=tri[d];[d]atrim=0:480,loudnorm=I=-19:TP=-2:LRA=7,volume=enable='between(t,13.5,29)':volume=0.45[m]" -map "[m]" -ar 44100 seg/music8.wav
ffmpeg -nostdin -loglevel error -y -i seg/music8.wav -i src/vo.mp3 -filter_complex "[1:a]adelay=14000|14000,loudnorm=I=-16:TP=-1.5:LRA=7,volume=0.9[v];[0:a][v]amix=inputs=2:duration=first:normalize=0,afade=t=out:st=476:d=4[a]" -map "[a]" -ar 44100 seg/mix.wav
ffmpeg -nostdin -loglevel error -y -i out/video.mp4 -i seg/mix.wav -c:v copy -c:a aac -b:a 192k -shortest -movflags +faststart out/episode.mp4
